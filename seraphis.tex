...
\end{enumerate}
The required security properties for application to Seraphis are described in Section \ref{sec-memb}. Specific proving systems satisfying the requirement include CSAG (CLSAG \cite{clsag} without linking) and One-out-of-Many proving system adapted from Groth and Bootle \textit{et al.} \cite{groth, bootle}.
...
The required security properties for application to Seraphis are described in Section \ref{ZKP}. Specific proving systems satisfying the requirement include Bulletproofs \cite{bp} and Bulletproofs+ \cite{bp-plus}.
...
We require some proving systems to be used in Seraphis to be zero-knowledge. Starting with interactive protocol description, these interative proving systems must satisfy the following three security properties. The precise definitions of these properties are common and found, for instance, in \cite{groth}.
\begin{itemize}
    \item \textit{Perfect Completeness}: If the prover knows a witness to the statement, she should always be able to generate a proof for that statement that convinces the verifier.
    \item \textit{Special Soundness}: If the prover does not know a witness to the statement, she should never be able to generate a proof for that statement that convinces the verifier. This is formalized by saying that there exists an \textit{witness extractor} that has an ability to ``rewind time'' and make the prover answer several different challenges. From the valid (by verifier) answers of the prover, it should be possible for the witness extractor to extract a witness.
    \item \textit{Perfect Special Honest Verifier Zero-Knowledge}: The protocol should not reveal anything about the prover’s witness. This is formalized by saying that assuming that challenges are honestly generated, there exists a \textit{simulator} such that even without knowledge of a witness, the probability distribution of the simulator's valid (by verifier) transcripts (all data being sent and received in the protocol) is equal to the probability distribution of actual valid transcripts by a prover and a verifier.
\end{itemize}
Fiat-Shamir heuristic \cite{fiat-shamir} transforms interactive proving systems with the above properties into non-interactive (full) zero-knowledge proofs (NIZKPs) in the random oracle model. We require that the proving systems for Ownership and Unspentness proofs, and Range proofs are NIZKPs constructed through Fiat-Shamir heuristic.
\subsection{Authenticated symmetric encryption scheme}\label{sec-symm}
We require that the authenticated symmetric encryption scheme has the following properties: indistinguishable against chosen-plaintext attack (IND-CPA), indistinguishable against adaptive chosen-ciphertext attack (IND-CCA2), and key-private under chosen-ciphertext attacks (IK-CCA). \textbf{Todo: Need to study this!}

\subsection{Membership proof security properties}\label{sec-memb}
...
We first define commitment functions $\textsf{CommAmount}:\mathbb{F}\times\mathbb{F}\rightarrow\mathbb{G}$ and $\textsf{CommTag}:\mathbb{F}\times\mathbb{F}\setminus\{0\}\times\mathbb{F}\rightarrow\mathbb{G}\times\mathbb{G}$ to be needed later on, as follows:
\begin{align*}
\textsf{CommAmount}(a; x) &= x G + a H \ , \ x\xleftarrow{\$}\mathbb{F}\ \text{if}\ x\ \text{is not specified.} \\
\textsf{CommTag}(t_k, k_a, k_b) &= (t_k G + k_a X + k_b U, (k_b/k_a)U).
\end{align*}
...
\item $b_4:=1$ if for all $i\in\{1,\ldots,n\}$, $\textsf{CommTag}(t_{k,i}, k_{a,i}^o, k_{b,i}^o) = (K_i'^o, \tilde{K}_i)$ in $T$, else $:=0$.
...
The composition proving system is a sigma protocol for the relation:
\begin{multline*}
...
\end{multline*}
Notice the extra $\{K_{t1,i}\}_{i=1}^n$ in the relation. This should not affect the proving system's applicability for Ownership and Unspentness proof because all $y_i$ are still hidden in $K_{t1,i}$ (by the DL assumption) and the required relationships for $\{K_i\}_{i=1}^n$ and $\{\tilde{K}_i\}_{i=1}^n$ are still proven. Now the Prover only needs to produce one proof transcript for all $i$'s instead of one proof transcript for each $i$. The sigma protocol is based on the concise approach from \cite{clsag} to reduce proof sizes when constructing multiple proofs in parallel.
...
\begin{align*}
r_{a} &= \alpha_a - c \sum_{j=1}^{n}{\mathcal{H}_7(\{K_{t2,i}\}_{i=1}^n,K_{t2,j}) (x_j/y_j)} \\
r_{b} &= \alpha_b - c \sum_{j=1}^{n}{\mathcal{H}_8(\{\tilde{K}_i\}_{i=1}^n,\tilde{K}_j) (z_j/y_j)} \\
...
\begin{align*}
A_{a} &= r_a G + c \sum_{j=1}^{n}{\mathcal{H}_7(\{K_{t2,i}\}_{i=1}^n,K_{t2,j}) K_{t2,j}} \\
A_{b} &= r_b U + c \sum_{j=1}^{n}{\mathcal{H}_8(\{\tilde{K}_i\}_{i=1}^n,\tilde{K}_j) \tilde{K}_j} \\
A_{i} &= r_i K_i + c K_{t1,i}\ ,\ \forall i\in\{1,\ldots,n\}\\
\end{align*}
\end{enumerate}
We now prove that the protocol is perfectly complete, special sound, and perfect special honest-verifier zero knowledge, all if $G$, $X$, and $U$ are mutually independent and $\mathcal{H}$ is a random oracle.
...
For special soundness, consider two valid (by verifier) transcripts with distinct challenge values $c \ne c' \in \mathbb{F}$:
$$(A_a, A_b, \{A_i\}_{i=1}^n,c, r_a, r_b, \{r_i\}_{i=1}^n)$$
and
$$(A_a, A_b, \{A_i\}_{i=1}^n,c', r'_a, r'_b, \{r'_i\}_{i=1}^n)$$
Now assume that $G$, $X$, and $U$ are mutually independent. The first validation equation and the validity of two transcripts imply that \textbf{Todo.}

This completes the proof.\end{proof}

Fiat-Shamir heuristic \cite{fiat-shamir} transforms interactive proving systems with the above three properties into non-interactive (full) zero-knowledge proofs (NIZKPs) in the random oracle model. From the above sigma protocol description, applying Fiat-Shamir heuristic to the composition proving sysyem should be straightforward.

\section{Security proofs for Seraphis}\label{proofs}
The needed math and hardness assumptions would be formalized first.
...
\end{definition}
\noindent Definition \ref{negl} is from Katz \& Lindell \cite{katz-lindell}. \textbf{Todo: full cite.} We prove the following lemma:
...
\end{proof}
\noindent Lemma \ref{negl-exp} justifies the usage of finite number of ``breaks'' in proofs by reduction. For a start, the probability for breaking a hardness assumption $\textsf{HA}$ is a function of $\lambda$. For now, we denote this function as $\textsf{Pr}[\textsf{HA}(\lambda)]$. Hence, for $m>1$, the probability for breaking $\textsf{HA}$ $m$-times $\textsf{Pr}[\cap_{i=1}^{m}{\textsf{HA}(\lambda)}]\ge\prod_{i=1}^{m}{\textsf{Pr}[\textsf{HA}(\lambda)]}$. Now Lemma \ref{negl-exp} says that if $\textsf{Pr}[\textsf{HA}(\lambda)]$ is non-negligible in $\lambda$ (equivalently, for all negligible function $\textsf{negl}(\lambda)$, $\textsf{Pr}[\textsf{HA}(\lambda)]\ge\textsf{negl}(\lambda)$), then $\prod_{i=1}^{m}{\textsf{Pr}[\textsf{HA}(\lambda)]}$ must also be non-negligible in $\lambda$ and hence $\textsf{Pr}[\cap_{i=1}^{m}{\textsf{HA}(\lambda)}]$ is also non-negligible in $\lambda$.

\begin{definition}
Discrete Logarithm (DL) assumption holds relative to $\lambda$ if for every $\emph{\textsf{PPT}}$ adversary $\mathcal{A}$, there exists a negligible function $\emph{\textsf{negl}}(\lambda)$ such that
\begin{align*}
\emph{\textsf{Pr}}\left[
\begin{array}{c|c}
    \begin{gathered}
    ...
    &
    \begin{gathered}
        pp\leftarrow\emph{\textsf{Setup}}(1^{\lambda}); G,H\xleftarrow{\$}\mathbb{G}; \\
        x\in\mathbb{F}\leftarrow\mathcal{A}(pp, G, H) \\
    \end{gathered}
\end{array}
\right]
\le \emph{\textsf{negl}}(\lambda).
...
\noindent The following theorems... \textbf{Todo.}
