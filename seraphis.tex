...
\item $\mathcal{A}$ is provided $k^v, k_a^s, k_b^s\in\mathbb{F}$ to construct the address ${\tt addr}_{\mathcal{A}} = (K_{\mathcal{A}}^{dh}, K_{\mathcal{A}}^v, K_{\mathcal{A}}^s)$, and $\{(k_{a,i}^o, k_{b,i}^o, a_i, x_i)\}_{i=1}^n$ that makes $\mathcal{A}$ the owner of ${\tt enote}_{\mathcal{A}}=\{(C_i,K_i^o,m_i)\}_{i=1}^n$ in the ledger.
...
If $(\emph{\textsf{Setup}},\emph{\textsf{PedersenC}})$, $(\emph{\textsf{Setup}},\emph{\textsf{PedersenK}})$ and $(\emph{\textsf{Setup}},\emph{\textsf{LinkTag}})$ are all binding, then Seraphis is balanced.  
\end{theorem}
...
\begin{itemize}
\item $pp\leftarrow\textsf{Setup}(1^{\lambda})$.
\item $\mathcal{A}$ is provided a random $\{\mathbb{S}_i\}_{i=1}^n$ \textit{with} potential sender addresses ${\tt send}_0$ and ${\tt send}_1$ such that both connect to exactly one e-note each in every $\mathbb{S}_i$, amounts of those one e-note each in every $\mathbb{S}_i$: $\{a_{0, i}\}_{i=1}^n$ and $\{a_{1, i}\}_{i=1}^n$ for ${\tt send}_0$ and ${\tt send}_1$, respectively, and potential receiver addresses ${\tt recv}_0$ and ${\tt recv}_1$.
...
Computational binding is proven by showing that if DL assumption holds, then every $\textsf{PPT}$ adversary $\mathcal{A}$ can \textit{break} $\textsf{LinkTag}$ only with negligible probability, where breaking $\textsf{LinkTag}$ means finding a correct triple input $(k_a, k_b, t_k)$ for a given pair output $(K'^o, \tilde{K})$. 

The proof is by contraposition: Assume that an $\mathcal{A}$ can break $\textsf{LinkTag}$ with non-negligible probability. Let $A, B\xleftarrow{\$}\mathbb{G}$ (both $\ne 0$) be the group elements to find DL for. $\mathcal{A}$ performs a $\textsf{LinkTag}$ break for $(K'^o, \tilde{K})=(G, A)$, giving $t_k, k_a, k_b\in\mathbb{F}$ such that $G = t_k G + k_a X + k_b U$ and $A = (k_b/k_a)U$. Note that both $k_a$ and $k_b$ will never be $0$ because if one of them is, then $A \ne (k_b/k_a)U$, a contradiction. Then $\mathcal{A}$ performs another $\textsf{LinkTag}$ break, this time for $(K'^o, \tilde{K})=(G, B)$ (for a total of 2 breaks). Now we have $y_1, y_2\in\mathbb{F}$ such that $A=y_1 U$ and $B=y_2 U$. This implies $B = y_2(1/y_1)A$.
\end{proof}
\end{document}
