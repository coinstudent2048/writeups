...
\item $\mathcal{A}$ is provided $k^v, k_a^s, k_b^s\in\mathbb{F}$ to construct the address ${\tt addr}_{\mathcal{A}} = (K_{\mathcal{A}}^{dh}, K_{\mathcal{A}}^v, K_{\mathcal{A}}^s)$, and $\{(k_{a,i}^o, k_{b,i}^o, a_i, x_i)\}_{i=1}^n$ (where $n\in\textsf{poly}(\lambda)$) that makes $\mathcal{A}$ the owner of ${\tt enote}_{\mathcal{A}}=\{(C_i,K_i^o,m_i)\}_{i=1}^n$ in the ledger.
\item $\mathcal{A}$ can choose any receiver's address ${\tt addr}_{\mathcal{B}} = (K_{\mathcal{B}}^{dh}, K_{\mathcal{B}}^v, K_{\mathcal{B}}^s)$.
\item $\{(k_{a,i}'^o, k_{b,i}'^o, a'_i, x'_i)\}_{i=1}^n \leftarrow\mathcal{A}(pp, {\tt enote}_{\mathcal{A}}, \{(k_{a,i}^o, k_{b,i}^o, a_i, x_i)\}_{i=1}^n)$.
\item $T\leftarrow{\tt tx}(pp, {\tt addr}_{\mathcal{A}}, {\tt addr}_{\mathcal{B}}, {\tt enote}_{\mathcal{A}}, \{(k_{a,i}'^o, k_{b,i}'^o, a'_i, x'_i)\}_{i=1}^n)$. $\mathcal{A}$ spends all e-notes in ${\tt enote}_{\mathcal{A}}$ to send an amount $a_{send}$ to ${\tt addr}_{\mathcal{B}}$.
\item $b_0:=1$ if Verifier accepts $T$, else $:=0$.
\item $b_1:=1$ if $\exists i\in\{1,\ldots,n\}: (k_{a,i}'^o, k_{b,i}'^o) \ne (k_{a,i}^o, k_{b,i}^o)$, else $:=0$.
\item $b_2:=1$ if $\sum_{i=1}^n{a_i} < a_{send}$, else $:=0$.
...
\begin{itemize}
\item $pp\leftarrow\textsf{Setup}(1^{\lambda})$.
\item $\mathcal{A}$ is provided a random $\{\mathbb{S}_i\}_{i=1}^n$ (where $n\in\textsf{poly}(\lambda)$) \textit{with} potential sender addresses ${\tt send}_0$ and ${\tt send}_1$ such that both connect to exactly one e-note each in every $\mathbb{S}_i$, amounts of those one e-note each in every $\mathbb{S}_i$: $\{a_{0, i}\}_{i=1}^n$ and $\{a_{1, i}\}_{i=1}^n$ for ${\tt send}_0$ and ${\tt send}_1$, respectively, and potential receiver addresses ${\tt recv}_0$ and ${\tt recv}_1$.
\item $\mathcal{A}$ chooses the amount the potential senders would send: $a_{\mathcal{A}, 0} \le \sum_{i=1}^n{a_{0, i}}$ and $a_{\mathcal{A}, 1} \le \sum_{i=1}^n{a_{1, i}}$ for ${\tt send}_0$ and ${\tt send}_1$, respectively.
...
\item $b'\leftarrow\mathcal{A}(pp, T, \{( {\tt send}_j, {\tt recv}_j, \{a_{j,i}\}_{i=1}^n, a_{\mathcal{A},j})\}_{j\in \{0,1\}})$.
...
If both $(\emph{\textsf{Setup}},\emph{\textsf{CommAmount}})$ and $(\emph{\textsf{Setup}},\emph{\textsf{CommTag}})$ are hiding, both ownership and unspentness proof and range proof are perfect special HVZK, membership proof is witness indistinguishable, authenticated symmetric encryption scheme is IND-CCA2 and IK-CCA, and $\mathcal{H}$ is modeled as a random oracle, then Seraphis is private.  
\end{theorem}
...
\begin{itemize}
\item $pp\leftarrow\textsf{Setup}(1^{\lambda})$.
\item $\mathcal{A}$ is provided $k^v, k_a^s, k_b^s\in\mathbb{F}$ to construct the address ${\tt addr}_{\mathcal{A}} = (K_{\mathcal{A}}^{dh}, K_{\mathcal{A}}^v, K_{\mathcal{A}}^s)$, and ${\tt enote}_{\mathcal{A}}=\{(C_i,K_i^o,m_i)\}_{i=1}^n$ (where $n\in\textsf{poly}(\lambda)$) in the ledger that $\mathcal{A}$ owns.
\item $\mathcal{A}$ can choose $k'^v, k_a'^s, k_b'^s\in\mathbb{F}$ to construct the receiver's address ${\tt addr}_{\mathcal{B}} = (K_{\mathcal{B}}^{dh}, K_{\mathcal{B}}^v, K_{\mathcal{B}}^s)$ and can choose a victim address ${\tt addr}_{\mathcal{C}} = (K_{\mathcal{C}}^{dh}, K_{\mathcal{C}}^v, K_{\mathcal{C}}^s)$.
\item $\mathcal{A}$ is provided ${\tt enote}_{\mathcal{C}}$ (where $|{\tt enote}_{\mathcal{C}}|\in\textsf{poly}(\lambda)$), which is all the e-notes in the ledger connected to ${\tt addr}_{\mathcal{C}}$. Let $\{\tilde{K}_{\mathcal{C}}\}$ be all the would-be linking tags of e-notes in ${\tt enote}_{\mathcal{C}}$ when those are spent honestly by the owner of ${\tt addr}_{\mathcal{C}}$.
