...
For membership proofs, we need the following property which is based on the one from \cite{groth}:
\begin{definition}[Perfect Anonymity]
$(\emph{\textsf{Setup}}, \mathcal{P}, \mathcal{V})$ is perfectly anonymous for $\mathcal{R}$ if for all $\emph{\textsf{PPT}}$ adversary $\mathcal{A}$,
\begin{align*}
\emph{\textsf{Pr}}\left[
\begin{array}{c|c}
    \begin{gathered}
    ...
    &
    \begin{gathered}
        pp\leftarrow\emph{\textsf{Setup}}(1^{\lambda}); i_0, i_1 \in \{1,\ldots,s\}, \mathbb{S}^*\leftarrow\mathcal{A}(pp,\emph{\textsf{CorruptRing}}); \\
        b\xleftarrow{\$}\{0,1\}; (x, w)\in\mathcal{R}\leftarrow\mathcal{P}(pp, \mathbb{S}^*, i_b); \\
        tr\leftarrow\langle\mathcal{P}(pp,x,w), \mathcal{V}(pp,x)\rangle; b'\leftarrow\mathcal{A}(pp, x, tr)
    \end{gathered}
\end{array}
\right]
= \frac{1}{2}
\end{align*}
where $\emph{\textsf{CorruptRing}}$ is an oracle that gives the corresponding private data for all ring members, making the adversary's ring $\mathbb{S}^*$ corrupted, $s=|\mathbb{S}^*|$ is the anonymity size, and $\mathcal{P}$ sets $\pi = i_b$ for the membership proof and replaces the $\pi$th element of $\mathbb{S}^*$ with its own (see Section \ref{mem} for details).
\end{definition}

All proving systems for application to Seraphis must \textit{at least} have perfect completeness and computational soundness. Moreover, Ownership and Unspentness proofs and Range proofs must at least have perfect special HVZK, while Membership proofs must at least have perfect anonymity.
