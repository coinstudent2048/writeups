...
\begin{abstract}
This document contains a concise description of Seraphis \cite{seraphis}, a novel privacy-preserving transaction protocol abstraction, and its security model.
\end{abstract}
...
\end{definition}
$K^{dh}$ is called the \textbf{Diffie-Hellman base public key}. The reason for its name will be clear in the next subsection. The $v$ superscript indicates ``view'', while the $s$ superscript indicates ``spend''. The reason for these names is irrelevent here, but see ``Addressing Schemes`` in \cite{seraphis}, Subsection 4.6. We say that someone \textit{owns} a receiver address if they know the corresponding scalars $k^v, k_a^s, k_b^s \in\mathbb{F}$.
...
It is important to note that the properties in Subsection \ref{prov-prop} are the \textit{minimal} requirements, thus the properties presented in this subsection may be weaker than in \cite{omniring}. Proofs for these properties are found in Appendix \ref{proofs}. Because of Seraphis's modularity, stronger properties for proving systems must yield stronger properties for Seraphis.
...
\item $T\leftarrow{\tt tx}(pp, {\tt addr}_{\mathcal{A}}, {\tt addr}_{\mathcal{B}}, {\tt enote}_{\mathcal{A}}, {\tt enote}_{\neg\mathcal{A}}, \{(k_{a,i}'^o, k_{b,i}'^o, a'_i, x'_i)\}_{i=1}^n)$. $\mathcal{A}$ spends $n$ e-notes to send an amount $a_t$ to ${\tt addr}_{\mathcal{B}}$.
...
\end{multline*}
Now the prover only needs to produce one proof transcript for all $i$ instead of one proof transcript for each $i$. This protocol is based on the concise approach from \cite{clsag} to reduce proof sizes when constructing multiple proofs in parallel.
...
\end{align*}
\end{enumerate}
We now prove that the protocol is perfectly complete, computationally sound, and witness indistinguishable, all if $G$, $X$, and $U$ are mutually independent.
...
For witness indistinguishability, [UNFINISHED]
\end{proof}
...
The following definitions are only applicable in membership proofs. [UNFINISHED]
